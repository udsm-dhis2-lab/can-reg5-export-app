...
const eventsQuery = {
    results: {
        ...
        params: ({
            ...
            program: 'rx6V962E4XM',
            fields: [
                'attributes[attribute,value],enrollments[events[storedBy,event,programStage,dataValues[dataElement,value]]]',
            ],
        ...
        trackedEntityInstances.map(tei => {
            ...
            for (let i = 0; i < tumourEvents.length; i++) {
                ...
                teiEvent.dataValues.map(dataValue => {
                    ...
                    if (dataValue.dataElement == 'w3hjoxhRdxX') {
                        DATEHIVTEST = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'Lklmhjoa2VZ') {
                        AGE = dataValue.value
                    }
                    if (dataValue.dataElement == 'YjyatbcXrAB') {
                        ADDR = dataValue.value
                    }
                    if (dataValue.dataElement == 'R7C6qavR1By') {
                        SECTOR = dataValue.value
                    }
                    if (dataValue.dataElement == 'JoiKTef007f') {
                        CELL = dataValue.value
                    }
                    if (dataValue.dataElement == 'hNjuN29oWEo') {
                        VILLAGE = dataValue.value
                    }
                    if (dataValue.dataElement == 'qiPi86HJH9D') {
                        INCID = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'b4nlCulDaNv') {
                        BAS = dataValue.value
                    }
                    if (dataValue.dataElement == 'mIGq36ORtj5') {
                        TOP = dataValue.value
                    }
                    if (dataValue.dataElement == 'R3V4FZ7bm1Z') {
                        BEH = dataValue.value
                    }
                    if (dataValue.dataElement == 'pUcbnDZTKWO') {
                        LATERALITY = dataValue.value
                    }
                    if (dataValue.dataElement == 'g4InB94akRh') {
                        MOR = dataValue.value
                    }
                    if (dataValue.dataElement == 'MiCTO3OgRB8') {
                        GRDE = dataValue.value
                    }
                    if (dataValue.dataElement == 'lsyoWxLKpcg') {
                        dataValue.value == 'Unkwown'
                            ? (STAGE = 'XX')
                            : (STAGE = dataValue.value)
                    }
                    if (dataValue.dataElement == 'jufaPpTt33C') {
                        T = dataValue.value
                    }
                    if (dataValue.dataElement == 'crCh4AWyhEQ') {
                        N = dataValue.value
                    }
                    if (dataValue.dataElement == 'YU85aZgUvpI') {
                        M = dataValue.value
                    }
                    TUMOURUPDATEDBY = teiEvent.storedBy ? teiEvent.storedBy : ''
                    if (dataValue.dataElement == 'QDYFCDo0kLm') {
                        INITIALT = dataValue.value
                    }
                    if (dataValue.dataElement == 'EJi6tdw5T1v') {
                        INTENTT = dataValue.value
                    }
                    if (dataValue.dataElement == 'lb1iN94cSNn') {
                        SGRY = dataValue.value
                    }
                    if (dataValue.dataElement == 'NzUoPBTcUme') {
                        formatCanRegDate((DATES = dataValue.value))
                    }
                    if (dataValue.dataElement == 'tmy8Js2OerA') {
                        CHEMO = dataValue.value
                    }
                    if (dataValue.dataElement == 'dqLzVzpPBQk') {
                        STARTC = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'qKApg9EbBvP') {
                        ENDCHEMO = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'qhTmdhweTY6') {
                        IMMUNO = dataValue.value
                    }
                    if (dataValue.dataElement == 'UH0QjAVVpBw') {
                        STARTI = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'Os8vbHJ3qoc') {
                        ENDIMMUNO = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'simuoODFRUc') {
                        RADIO = dataValue.value
                    }
                    if (dataValue.dataElement == 'KuewOYQYRq7') {
                        STARTR = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'THtdWv46cXH') {
                        ENDRADIO = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'QKXwZ57aGdH') {
                        HORMO = dataValue.value
                    }
                    if (dataValue.dataElement == 'pazmxkluuAK') {
                        STARTH = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'ZTtTjPPKemm') {
                        ENDHORMO = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'N6J5Bp9auN9') {
                        PALLIA = dataValue.value
                    }
                    if (dataValue.dataElement == 'KWsp9YpTp8O') {
                        DATEP = formatCanRegDate(dataValue.value)
                    }
                    if (dataValue.dataElement == 'YYW855k5GgW') {
                        OTHERT = dataValue.value
                    }
                    if (dataValue.dataElement == 'tfeZgkgqJC9') {
                        SPECIFYOT = dataValue.value
                    }
                })
            }
            ...
            for (let i = 0; i < sourceEvents.length; i++) {
                ...
                teiEvent.dataValues.map(dataValue => {
                    ...
                    if (dataValue.dataElement == 'v9h8LhYlF2k') {
                        source_service = dataValue.value
                    }
                    if (dataValue.dataElement == 'gfyCp3UGFBg') {
                        patient_hospital_number = dataValue.value
                    }
                    if (dataValue.dataElement == 'g5PfzRwNHVy') {
                        archive_code = dataValue.value
                    }
        ...
        <div className={classes.tableContainer}>
            <div className="products">
                <AllRecordsHeaderView
                    ...
                    provinces={data.provinces.children}
                />
